...
# from detection_x6.model import x6
...
def run(args, conf_thres, iou_thres, sources, result_paths, perspective, cam_ids, scene):
    ...
    with open(f'./before_offline/{scene}_x6/{scene}.txt', 'w') as lf:
        lf.write('')
    ...
    # detection = x6()
    # detection.initialize(f'detection_x6/{scene}_x6.json')
    ...
    while True:
        ...
        for (img_paths, writer), tracker, perspective_transform, result_list in zip(src_handlers, trackers, perspective_transforms, results_lists):
            if len(img_paths) == 0:
                stop = True
                break
            if cur_frame < 3600 and cur_frame>100:
                img_paths.pop(0)
                continue
...
if __name__ == '__main__':
    ...
    run(args=args, conf_thres=0.1, iou_thres=0.45, sources=sources['S005'], result_paths=result_paths['S005'], perspective='S005', cam_ids=cam_ids['S005'], scene='S005')
